...
<html>
    ...
    <body>
        ...
        <h2 style="text-align: center; font-family: Sofia; color: white" >Trader Registration </h2>


<div style="background-color: white; align: center; text-align:center; width: 50%;display: table; margin: 0 auto;" >
	<br />
	<form action="trader_register.php" method="post" class="mui-form" enctype="multipart/form-data" > 
		...
		<div class="mui-textfield mui-textfield--float-label">
			<input type="file" name="c_image" class="mui-btn mui-btn--raised" required>
			<label>Image</label>
		</div>
		...
		<div class="mui-checkbox">
			...
			<button type="submit" class="mui-btn mui-btn--raised" name="register" id="trader_registerButton" >Submit</button>
...
<?php
	...
	if (isset($_POST['register'])) {
		...
		$checkEmail = oci_parse($connection, "SELECT * FROM trader WHERE email='$c_email'");
			if (!$checkEmail) {
        				...
        				if(preg_match("#[0-9]+#",$c_password)) {
        					if(preg_match("#[A-Z]+#",$c_password)) {
        						if (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $c_password)) {
									...
									// $encrypt_pasword = $c_password;		
							
									// $run_trader = mysqli_query($con, $insert_trader);
					$insert_trader = oci_parse($connection, "INSERT INTO trader (traderId, 
																				firstName, 
																				lastName, 
																				age, 
																				sex, 
								...
								$r = oci_execute($insert_trader);
								if (!$r) {
									$e = oci_error($insert_trader);
									trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
								}
            ...
            $message .= '<br />';
            $message .= '<p style="text-align:center;font-family:Comic Sans MS">We are delighted to have you as a new trader on our webpage. We are truely Thankful</p>';
            $message .= '<p style="text-align:center;font-family:Comic Sans MS">Please follow this link to sign up</p>';
            $message .= '<a style="text-align:center" href="http://localhost/bronte/admin/traderLogin/trader_login.php" ><p style="text-align:center;font-family:Comic Sans MS" >Click me to Sign In</span></p>';
            $message .= '<br />';
            $message .= '<br />';
...
font-family: arial; text-decoration: none;">If you no longer want to receive our emails, please <a href="default.asp">un-subscribe here</a>.</span>';
		    ...
		    mail($to, $subject, $message, $headers);
								...
								echo "<script>alert('Registration Successful')</script>";
									...
									oci_free_statement($insert_trader); 
								}else{
