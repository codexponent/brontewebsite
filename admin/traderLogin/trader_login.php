...
<html>
    ...
    <body>
                ...
                <header class="mdl-layout__header mdl-layout__header--transparent">
                    ...
                    <nav class="mdl-navigation">
                        ...
                        <a class="mdl-navigation__link" href="../../products.php">Products</a>
                         <a class="mdl-navigation__link" href="../login/admin_login.php">Admin</a>
                        <a class="mdl-navigation__link" href="#">Trader</a>
        ...
        <h2 style="text-align: center; font-family: Sofia; color: white" >Trader Login </h2>


<div style="background-color: white; align: center; text-align:center; width: 50%;display: table; margin: 0 auto;" >
	<br />
	<form action="trader_login.php" method="post" class="mui-form" > 
...
<a href="trader_register.php" ><button class="mui-btn mui-btn--raised">Register</button></a>
<button type="submit" class="mui-btn mui-btn--raised" name="login" id="trader_loginButton" >Submit</button>
...
<br /><br /><br /><br /><br />
	...
	<?php
		...
		if (isset($_POST['login'])) {
            ...
            //Changed here - Major Change Here----------------------------------<--<-<-<-<-<-<-
            // $encrypt_password = md5($c_password);
            // $encrypt_password = $c_password;

            if ($c_password == '&Trader1001' || '&Trader1002' || '&Trader1003' || '&Trader1004' || '&Trader1005' ){
                $encrypt_password = $c_password;
            }else{
                $encrypt_password = md5($c_password);
            }
			...
			$checkEmail = oci_parse($connection, "SELECT * FROM trader WHERE password = '$encrypt_password' AND email = '$c_email'");
			if (!$checkEmail) {
			...
			$getId = oci_parse($connection, "SELECT traderId FROM trader where email = '$c_email'");
			if (!$getId) {
                $e = oci_error($connection);
                trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
            }
			...
			echo "<script>window.open('../shop/view_shops.php?view_shops=$id', '_self')</script>";
